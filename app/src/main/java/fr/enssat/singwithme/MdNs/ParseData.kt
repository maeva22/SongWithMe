...
fun tabToString(textsong: Karaoke) : String {
    var tps = ""
    for (value in textsong.lyrics) {
        tps += value.text + "\n"
    }
    return tps
}
...
fun transformToData(textsong:String){
    val regexRule  ="""(?=(?>\{ *([^\n {}]+) *}([^\n{}]+\n?)(?>\{ *([^\n {}]+) *}(\n)?)))"""
    ...
    for(pack in result){
        ...
        tab.add(
            ParoleParse(txt = pack.groups[2]!!.value.replace('\n',' ') , timerStart =timeCodeToMs(pack.groups[1]!!.value) , timerEnd =timeCodeToMs(pack.groups[3]!!.value), end =check )
        )
    }
